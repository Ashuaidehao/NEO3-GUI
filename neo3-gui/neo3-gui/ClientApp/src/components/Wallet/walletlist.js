  ...
  showModal = (ele) => {
    ...
    return () => {
      ...
      switch (ele) {
        case 0:
          this.setState({
            modalPanel: <Private func={this.handleCancel} />,
            modalTitle: t("wallet.import private"),
          });
          break;
        case 1:
          this.setState({
            modalPanel: <Multiaddress func={this.handleCancel} />,
            modalTitle: t("wallet.signature multi"),
          });
          break;
        default:
    ...
    Array.call([], ...accounts).map(function (item) {
      ...
      switch (_item.accountType) {
        case 0:
          ...
          break;
        case 1:
          normaladd.push(_item);
          break;
        case 2:
          mutiadd.push(_item);
          break;
        case 3:
          contractadd.push(_item);
          break;
      }
    });
        ...
        <Content className="mt3">
          ...
          >
            <Col span={13} className="bg-white pv4">
                ...
                <h2 className="mb0">
                  ...
                  <div className="wal-import float-r">
                    ...
                    <div className="wal-ul">
                      <ul>
                        <li>
                          <a onClick={this.addAddress}>
                            {t("wallet.add address")}
                          </a>
                        </li>
                        <li>
                          <a onClick={this.showModal(0)}>
                            {t("wallet.import private")}
                          </a>
                        </li>
                        <li>
                          <a onClick={this.showModal(1)}>
                            {t("wallet.signature multi")}
                          </a>
                        </li>
                      </ul>
                    </div>
              ...
              <Accounts
                accounts={normaladd}
                name={t("wallet.address standard")}
              />
              <Accounts
                accounts={mutiadd}
                name={t("wallet.address multi sign")}
              />
              <Accounts
                accounts={contractadd}
                name={t("wallet.address contract")}
              />
            ...
            <Col span={10} offset={1} className="bg-white pv4">
              ...
              <List
                ...
                header={
                  <div>
                    <span>
                      {t("blockchain.asset info")} <small></small>
                    </span>
                    <span className="float-r wa-amount">
                      {t("wallet.balance")}
                    </span>
                  </div>
                }
                  ...
                  <List.Item>
                    <List.Item.Meta
                      avatar={
                        <Avatar
                          src={
                            "https://neo.org/images/gui/" + item.asset + ".png"
                          }
                        />
                      }
                      title={
                        <span className="upcase" title={item.asset}>
                          {item.symbol}
                        </span>
                      }
              ...
              <div className="w200 mt4">
                <Button
                  className="w200"
                  onClick={this.claimGas}
                  loading={this.state.iconLoading}
                >
                  {t("button.claim")} {unclaimedGas} GAS
                </Button>
              </div>
            </Col>
          </Row>
...
const Private = ({ func }) => {
  ...
  const importPrivate = (values) => {
    post("ImportAccounts", [values.private])
      .then((res) => {
        let _data = res.data;
        if (_data.msgType === 3) {
  ...
  return (
    <Form className="neo-form" form={form} onFinish={importPrivate}>
      <Form.Item
        name="private"
        rules={[
          { required: true, message: t("please input Hex/WIF private key") },
        ]}
      >
...
const Multiaddress = ({ func }) => {
  ...
  const addMulti = (values) => {
    ...
    post("CreateMultiAddress", params)
      .then((res) => {
        var _data = res.data;
        if (_data.msgType === -1) {
          ...
          Modal.success({
            ...
            content: (
              <div className="show-pri">
                <p>
                  <Trans>hash</Trans> ：{_data.result.scriptHash}
                </p>
                <p>
                  <Trans>wallet.address multi sign</Trans> ：
                  {_data.result.address}
                </p>
              </div>
            ),
  ...
  return (
    <Form className="neo-form" form={form} onFinish={addMulti}>
      ...
      <Form.Item
        name="publicKeys"
        rules={[
          { required: true, message: t("wallet.please input public key") },
        ]}
      >
        <Select
        ...
        >
          {accounts.length > 0
            ? accounts.map((item) => {
                console.log({ ...item });
                return (
                  <Option className="add-list" key={item.publicKey}>
                    {item.publicKey}
                    <span className="add-show">{item.address}</span>
                  </Option>
                );
              })
            : null}
      ...
      <Form.Item
        name="limit"
        rules={[
          { required: true, message: t("wallet.please input signature min") },
        ]}
      >
        <InputNumber
      ...
      renderItem={(item) => (
        <List.Item>
          <List.Item.Meta
            title={
              <Link
                to={"/wallet/walletlist:" + item.address}
                title={t("wallet.show detail")}
              >
                {item.address}
              </Link>
            }
            description={
              <span className="f-s">
                <span className="amount mr2">
                  NEO <span className="wa-count">{item.neo}</span>
                </span>
                <span>
                  GAS <span className="wa-count">{item.gas}</span>
                </span>
              </span>
            }
          />
